	...
	Vector2f disparityCalibParams, float fx_depth, Vector2i imgSize)
{
	...
	//short disparity = d_in[locId];
	//
	//float min_disp = 1.0f / disparityCalibParams.y;
	//float max_disp = 1.0f / disparityCalibParams.x;

	//float scale_factor = 10000.0f;
	//float disparity_tmp = ( ( (float)disparity / scale_factor ) * (max_disp - min_disp) ) + min_disp;
	//float depth;

	//if (disparity_tmp == 0) depth = 0.0f;
	//else depth = 1.0f / disparity_tmp;

	//d_out[locId] = ((depth <= 0) || (depth > disparityCalibParams.y)) ? -1.0f : depth;
	int locId = x + y * imgSize.x;
